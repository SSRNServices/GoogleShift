...
describe('DriveService - getSelectionSummary', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should correctly count a single file', async () => {
    mockFilesGet.mockResolvedValueOnce({
      data: { name: 'test.txt', size: '100' }
    });

    const summary = await driveService.getSelectionSummary(
      'source',
      [{ id: 'file1', isFolder: false }],
      onProgress
    );
