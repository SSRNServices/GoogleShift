...
export class ManifestStorage {
  ...
  public static async insertItem(item: ManifestItem) {
    ...
    await db.run(`
      ...
      (jobId, id, sourceId, sourceParentId, destParentId, createdDestId, name, mimeType, size, originalId, originalMimeType, status, isFolder)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
